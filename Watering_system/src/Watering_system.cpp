/* 
 * Project Smarty Plants Watering System
 * Author: Jay Elder
 * Date: 03/19/2026
 */
...
IoTTimer pumpTimer;
HX711 myScale (D2, D5);  //pins connected to hx711

const int soilSensor = A5;
const int pump = D9;
const int CALFACTOR = 700; 
const int SAMPLES = 10;
const int DRY_THRESHOLD = 1400;
...
void loop() {
    ...
    while ((subscription = mqtt.readSubscription(100))) {
        ...
        if (!wateringEnabled) {
            digitalWrite(pump, LOW);
            Serial.printf("Auto watering blocked: low reservoir.");
        }
